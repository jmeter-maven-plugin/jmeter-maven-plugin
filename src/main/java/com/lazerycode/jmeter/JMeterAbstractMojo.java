...
public abstract class JMeterAbstractMojo extends AbstractMojo {
	...
	/**
	 * Append the results timestamp to the filename (It will be prepended by
	 * default if testResultsTimestamp is set to true)
	 */
	 ...
	 * (This assumes that testResultsTimestamp is set to 'true') For formatting
	 * see http://joda-time.sourceforge.net/apidocs/org/joda/time/format/
	 * DateTimeFormat.html
	 */
	@Parameter()
	...
	/**
	 * Set the format of the results generated by JMeter Valid values are: xml,
	 * csv (XML set by default).
	 */
	...
	/**
	 * Value class that wraps all remote junit configurations.
	 */
	@Parameter
	...
	/**
	 * Set a root log level to override all log levels used by JMeter Valid log
	 * levels are: FATAL_ERROR, ERROR, WARN, INFO, DEBUG (They are not case
	 * sensitive); If you try to set an invalid log level it will be ignored
	 */
	@Parameter
	 ...
	 * <testFilesDirectory> Defaults to "logkit.xml"
	 */
	...
	protected void generateJMeterDirectoryTree() {
		...
		if (null != resultsDirectory) {
			resultsDir = new File(resultsDirectory.replaceAll("\\|/",
					File.separator));
		} else {
	...
	protected void propertyConfiguration() throws MojoExecutionException {
		pluginProperties = new PropertyHandler(testFilesDirectory, binDir,
	...
	/**
	 * Create the JMeter directory tree and copy all compile time JMeter
	 * dependencies into it. Generic compile time artifacts are copied into the
	 * libDir ApacheJMeter_* artifacts are copied into the libExtDir Runtime
	 * dependencies set by the user are also copied into the libExtDir
	 * 
			...
			try {
				if (Artifact.SCOPE_COMPILE.equals(artifact.getScope())
					...
					if (artifact.getArtifactId().equals(jmeterConfigArtifact)) {
						...
						copyFile(artifact.getFile(), new File(binDir
								+ File.separator + artifact.getArtifactId()
								+ ".jar"));
					} else if (artifact.getArtifactId().startsWith(
							"ApacheJMeter_")) {
						copyFile(artifact.getFile(),
								new File(libExtDir + File.separator
										+ artifact.getFile().getName()));
					} else if (isArtifactAJMeterDependency(artifact)) {
						copyFile(artifact.getFile(),
								new File(libDir + File.separator
										+ artifact.getFile().getName()));
					} else if (isArtifactAnExplicitDependency(artifact)) {
						if (isArtifactMarkedAsAJMeterPlugin(artifact)) {
							copyFile(artifact.getFile(), new File(libExtDir
									+ File.separator
									+ artifact.getFile().getName()));
						} else if (isArtifactMarkedAsAJUnitPlugin(artifact)) {
							copyFile(artifact.getFile(), new File(libJUnitDir
									+ File.separator
									+ artifact.getFile().getName()));
						} else {
							copyFile(artifact.getFile(), new File(libDir
									+ File.separator
									+ artifact.getFile().getName()));
						}
					}
	...
	/**
	 * Extract the configuration settings (not properties files) form the
	 * configuration artifact and load them into the /bin directory
	 * 
	 * @param artifact
	...
	private void extractConfigSettings(Artifact artifact) throws IOException {
		...
		while (entries.hasMoreElements()) {
			...
			// Only interested in files in the /bin directory that are not
			// properties files
			if (!jarFileEntry.isDirectory()
					&& jarFileEntry.getName().startsWith("bin")
					&& !jarFileEntry.getName().endsWith(".properties")) {
				File fileToCreate = new File(workDir.getCanonicalPath()
						+ File.separator + jarFileEntry.getName());
				if (jarFileEntry.getName().endsWith(logConfigFilename)
						&& fileToCreate.exists()) {
					break;
				}
				copyInputStreamToFile(
						configSettings.getInputStream(jarFileEntry),
						fileToCreate);
			}
		}
	...
	/**
	 * Check if the given artifact is needed by an explicit dependency (a
	 * dependency, that is explicitly defined in the pom of the project using
	...
	protected boolean isArtifactAnExplicitDependency(Artifact artifact) {
		try {
			// Maven 3
			List<Dependency> pluginDependencies = mojoExecution.getPlugin()
					.getDependencies();
			for (Dependency dependency : pluginDependencies) {
				for (String parent : artifact.getDependencyTrail()) {
					if (parent.contains(dependency.getGroupId() + ":"
							+ dependency.getArtifactId())
							&& parent.contains(dependency.getVersion())) {
						return true;
					}
				}
			}
			...
			// Maven 2
			Set<Artifact> pluginDependentArtifacts = mojoExecution
			...
			for (Artifact dependency : pluginDependentArtifacts) {
				for (String parent : artifact.getDependencyTrail()) {
					if (parent.contains(dependency.getGroupId() + ":"
							+ dependency.getArtifactId())
							&& parent.contains(dependency.getBaseVersion())) {
						return true;
					}
				}
			}
	 ...
	 * @throws MojoExecutionException
	 */
	protected Artifact getArtifactNamed(String artifactName)
			throws MojoExecutionException {
		for (Artifact artifact : pluginArtifacts) {
	...
	protected void initialiseJMeterArgumentsArray(boolean disableGUI)
			throws MojoExecutionException {
		testArgs = new JMeterArgumentsArray(disableGUI,
		...
		if (testResultsTimestamp) {
			...
			if (isSet(resultsFileNameDateFormat)) {
				try {
					testArgs.setResultsFileNameDateFormat(DateTimeFormat
							.forPattern(resultsFileNameDateFormat));
				} catch (Exception ex) {
					getLog().error(
							"'"
									+ resultsFileNameDateFormat
									+ "' is an invalid DateTimeFormat.  Defaulting to Standard ISO_8601.");
				}
			}
		}
	...
	protected void setJMeterResultFileFormat() {
		if (resultsFileFormat.toLowerCase().equals("csv")) {
			propertiesJMeter
					.put("jmeter.save.saveservice.output_format", "csv");
			resultsOutputIsCSVFormat = true;
		} else {
			propertiesJMeter
					.put("jmeter.save.saveservice.output_format", "xml");
			resultsOutputIsCSVFormat = false;
		}
	}

	protected void configureAdvancedLogging() throws MojoFailureException {
		File advancedLoggingSetting = new File(testFilesDirectory
				+ File.separator + logConfigFilename);
		if (advancedLoggingSetting.exists()) {
			try {
				copyFile(advancedLoggingSetting, new File(binDir
						+ File.separator + logConfigFilename));
